<x-dialog-modal-cotizacion wire:model="open3">

    <x-slot name="title" >
        <div class="bg-blanco w-full py-2 rounded-t-3xl border-principal border-b-4 grid grid-cols-12 gap-4 px-6 lg:px-12">
            <p class="col-span-1 text-[3.5rem] font-semibold text-oscuro text-start">3</p>
        ...
        <div class="bg-blanco w-full rounded-b-3xl px-6">
            <div class="pt-4 text-xl font-bold  rounded-t-3xl text-oscuro text-center">
                <button class="w-full bg-blanco text-lg lg:text-xl font-semibold border-principal border lg:border-2 rounded-xl text-principal hover:text-principal hover:bg-claro hover:border-principal py-4 mb-4" wire:click="$set('nuevo3', true)">AGREGAR NUEVO DETALLE</button>

                @include('livewire.admin.cotizaciones.editar.inputs.input3')
            </div>
        </div>
    </x-slot>
            ...
            <div class="scroll-container overflow-x-auto">
                <div class="grid grid-cols-12 bg-blanco p-4 text-start  text-lg w-full min-w-[480px] gap-4 border-b border-medioClaro">
                    ...
                    <div class="col-span-12 text-medio font-normal text-xl px-6">
                        Los últimos detalles creados
                    </div>
                </div>

                @foreach($detallesCotizacion as $indice => $item)
                    ...
                    <div class="grid grid-cols-12 bg-blanco font-normal text-oscuro text-md p-4  w-full min-w-[480px] gap-4 border-b border-medioClaro">
                        ...
                        <div class="col-span-3 flex justify-start items-center ">
                            <button wire:click="editarDetalle('{{$indice}}');"><i class="ri-edit-line text-2xl text-principal transition-all ease-in-out duration-500 hover:blur-[1px] mr-4"></i></button>
                            <button wire:click="borrarDetalle('{{$indice}}')"><i class="ri-delete-bin-line text-2xl transition-all ease-in-out duration-500  hover:blur-[1px]  text-danger"></i></button>
                        </div>
                    </div>
                @endforeach

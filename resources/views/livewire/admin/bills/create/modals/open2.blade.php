<div
    x-data="{}"
    x-on:keydown.escape.window="open2 = false; activar = true; Livewire.emit('reinicioVariables');"
    x-show="open2"
    class="jetstream-modal fixed top-12 inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50"
    style="display: none;"
    >
    <div x-show="open2" class="fixed inset-0 transform transition-all " x-on:click="open2 = false; activar = true; Livewire.emit('reinicioVariables');" x-transition:enter="ease-out duration-300"
        ...
        <div class="bg-blanco w-full p-4 px-6 lg:px-12 rounded-b-3xl  ">
            <div x-data="{ inputValue2: (this.producto_id == undefined ? '' : this.producto_id ) , isFocused2: false,  producto_id: @entangle('producto_id'),  productos: @entangle('productos2') , isShow: true , select: function() { this.clienteId = this.inputValue2} ,  itemDescripcion: @entangle('itemDescripcion'), itemCantidad: @entangle('itemCantidad'),  itemLargo: @entangle('itemLargo'), itemAncho: @entangle('itemAncho')}" @click.away = "isFocused2 = false">
                ...
                <div class="grid grid-cols-2 gap-4 mt-8 mb-2">
                    <button @click="open2= false ; if(inputValue2 != '') {icon1 = true; console.log('true')} else {icon1 = false ; console.log('false') }; activar = true; Livewire.emit('reinicioVariables'); "
